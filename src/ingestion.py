...
class VideoInfo:
    ...
    product: str = ""  # 产品名称（从目录结构提取）


def _extract_product(input_dir: str, filepath: str) -> str:
    """从文件路径提取产品名称

    目录结构: input/{product}/import_{date}/video.mp4
    或旧格式: input/video.mp4（无产品）
    """
...
def scan_input_dir(config: Config) -> List[VideoInfo]:
    """扫描输入目录（递归），返回视频文件信息列表

    支持两种目录结构:
    - 旧格式: input/video.mp4
    - 新格式: input/{product}/import_{date}/video.mp4
    """
    ...
    for root, dirs, files in os.walk(input_dir):
        ...
        for filename in sorted(files):
            ...
            product = _extract_product(input_dir, filepath)
